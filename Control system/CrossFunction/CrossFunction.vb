...
Public Module CrossFunction
	...
	Public Sub CheckCross()
		For iAGV As Byte = 0 To AGVArray.Length - 1
			AGVCrossCheck(iAGV)
		Next
	End Sub
	Public Sub initCrossVar()
		AGVCrossFlag = New Boolean(AGVArray.Length - 1, CrossArray.Length - 1) {}
		...
		For CrossPositionIndex = 0 To MAX_CROSS - 1
			If isInCross(myAGVNumber, CrossPositionIndex) Then
				If CrossArray(CrossPositionIndex).isExistAGV = True Then	'Neu da co tg trong cross
					...
					If AGVCrossFlag(myAGVNumber, CrossPositionIndex) = False And AGVArray(myAGVNumber).Status = AGV.RobocarStatusValue.NORMAL Then
						AGVArray(myAGVNumber).AGVStop()
						AGVArray(myAGVNumber).Status = AGV.RobocarStatusValue.STOP_BY_CARD
					End If
				Else 'Neu ko co tg nao trong cross
					...
					If AGVArray(myAGVNumber).Status = AGV.RobocarStatusValue.STOP_BY_CARD Then
						AGVArray(myAGVNumber).AGVRun()
						AGVArray(myAGVNumber).Status = AGV.RobocarStatusValue.NORMAL
					End If
				End If
				AGVCrossFlag(myAGVNumber, CrossPositionIndex) = True
			Else
				If AGVArray(myAGVNumber).Position <> 0 Then	'It mean RBC position different with all card in CardInArray
